...
class MyAgent701(Snake):
    ...
    def updateDirection(self,maze):
        ...
        if(self.pathlen(enemy_head,maze.foodpos) + 5 < shortest):
           # print("AVOID")
            path = self.avoid_food(position, self.direction, maze, begin_time)
            self.direction= path[-1].dir if path and path[-1] in validdir else olddir
            self.last = None
            self.closedNodes = []
            self.food_found = False
    ...
    def avoid_food(self, startPos, startDir, maze, begin_time):
        tail = self.body[-1]
        target_not_food_pos = (self.add(tail, (-5 * startDir[0], -5 * startDir[1])))
        return self.aa_regular(startPos, startDir, maze, begin_time)
