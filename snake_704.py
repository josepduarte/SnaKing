...
class MyAgent704(Snake):
    ...
    def updateDirection(self,maze):
        ...
        # regular astart
            ...
            aastar_can_be_used = False
            path = self.aa_regular(position, self.direction, maze, begin_time)
            if self.aastar_can_be_used:
                dir = path[-1].dir if path and path[-1].dir in validdir else olddir
                dir = olddir
                self.last = None
                self.closedNodes = []
            else:
                # quer dizer que o aastar já foi usado uma vez mas deixou de conseguir chegar ao caminho
                if self.last == None:
                    self.food_found = False
                path = self.aa_improved(position, self.direction, maze, begin_time)
                if path and path[-1].dir in validdir:
